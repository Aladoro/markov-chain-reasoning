...
class GPSAC(SAC):
    def __init__(self, make_actor, make_critics,
        ...
        if initialize_model:
            ...
            if self._save_training_statistics:
                self._al_metric = tf.keras.metrics.Mean('al')
                self._ql_metric = tf.keras.metrics.Mean('ql')
                self._alpha_metric = tf.keras.metrics.Mean('alpha')
                self._unc_metric = tf.keras.metrics.Mean('unc')
        ...
        if actor_uncertainty_coeff_shift_decay_steps > 0:
            ...
            if decay_step < 0.0:
                def decay_actor_uncertainty_coeff_shift():
                    self._actor_optimistic_shift_value.assign(
                        tf.minimum(self._actor_optimistic_shift_value - decay_step, 0.0))
            elif decay_step > 0.0:
                def decay_actor_uncertainty_coeff_shift():
                    self._actor_optimistic_shift_value.assign(
                        tf.maximum(self._actor_optimistic_shift_value - decay_step, 0.0))
            else:
    ...
    def build(self, input_shape):
        self._observation_dims = len(input_shape) - 1
        super(GPSAC, self).build(input_shape)
    ...
    def save_metrics_and_reset(self, ):
        self._latest_log_dict = {'q_loss': self._ql_metric.result().numpy(),
                                 ...
                                 'unc': self._unc_metric.result().numpy(),
                                 'unc_coeff': self._uncertainty_coeff.numpy()}
