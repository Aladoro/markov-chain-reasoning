...
class StochasticActor(tf.keras.layers.Layer):
    def __init__(self, layers_list, norm_mean=None, norm_stddev=None, min_log_stddev=-10,
                 max_log_stddev=2, action_scale=1.0, correct_logprob_for_action_scale=False,):
        super(StochasticActor, self).__init__()
        self._act_layers = layers_list
        self._out_dim = self._act_layers[-1].units
        ...
        self.distribution_bijector = tfb.Chain([tfb.Tanh(),
                                                tfb.Scale(scale=self._action_scale)])
    ...
    def get_action_distribution(self, observation_batch):
...
class TruncStochasticActor(StochasticActor):
    def __init__(self, layers_list, norm_mean=None, norm_stddev=None, min_log_stddev=-5,
                 max_log_stddev=-0.5, action_scale=1.0, correct_logprob_for_action_scale=False,):
        super(StochasticActor, self).__init__()
        self._act_layers = layers_list
        self._out_dim = self._act_layers[-1].units
        self._action_dim = self._out_dim // 2
    ...
    def get_action(self, observation_batch, noise_stddev, *args, **kwargs):
        ...
        act = stable_sample_tn(loc=mean, scale=stddev, min=-1.0, max=1.0)
        return act * self._action_scale

    def get_action_probability(self, observation_batch):
        pre_obs_batch = self.preprocess_obs(observation_batch)
        mean, stddev = self.__call__(pre_obs_batch)
        actions = stable_sample_tn(loc=mean, scale=stddev, min=-1.0, max=1.0)
        dist = tfd.TruncatedNormal(mean, stddev, low=-1.0, high=1.0)
        # actions = dist.sample()
...
class SAC(tf.keras.Model):
    def __init__(self,
                 actor,
                 critics,
                 actor_optimizer=tfo.Adam(1e-3),
                 critic_optimizer=tfo.Adam(1e-3),
                 entropy_optimizer=tfo.Adam(1e-4),
                 gamma=0.99,
                 q_polyak=0.995,
        ...
        self._act = actor
        self._cri = critics
        self._targ_cri = copy.deepcopy(critics)
        self._targ_cri._name = 'target_' + self._targ_cri._name
    ...
    def call(self, inputs):
        ...
        out['q'] = self._cri.get_qs(inputs, out['act'])
        out['t_q'] = self._targ_cri.get_qs(inputs, out['act'])
        return out
    ...
    def make_actor_and_alpha_train_op(self, ):
        def q_estimator(observations, actions):
            return tf.reduce_min(self._cri.get_qs(observations, actions), axis=-1, keepdims=True)
        if self._tune_entropy_coefficient:
